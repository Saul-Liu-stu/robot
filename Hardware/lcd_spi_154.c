...
*					2.	可设置要显示的颜色，例如使用 LCD_SetColor(0xff0000FF) 设置为蓝色
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0xff000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayChinese( 10, 10, "STM32") ，在坐标(10,10)显示字符串"STM32"
*
**********************************************************************************************************************************LXB*******/
...
{  
	...
	while(*pText != 0)	// 判断是否为空字符
	{
		...
		{			
			LCD_DisplayText(x,y,"ERROR");	// 显示错误（暂时）
...
{  
	...
	{
		sprintf( Number_Buffer , "%.*d",len, (int)number );	// 将 number 转换成字符串，便于显示
	}
	else			// 多余位补空格
	{	
		sprintf( Number_Buffer , "%*d",len, (int)number );	// 将 number 转换成字符串，便于显示
...
// 因为缓冲区大小有限，需要分多次写入
   Buff_Height = (sizeof(LCD_Buff)/2) / width;    // 计算缓冲区能够写入图片的多少行
...
void 	LCD_DrawImageColorful(uint16_t x,uint16_t y,uint16_t width,uint16_t height,const uint8_t *pImage)
{
    uint16_t  disChar;	         // 字模的值
	uint16_t  Xaddress = x;       // 水平坐标
 	uint16_t  Yaddress = y;       // 垂直坐标
