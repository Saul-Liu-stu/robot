...
void LCD_Test_Clear(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		LCD_Clear();		// 清屏
		LCD_DisplayText(13, 70,"STM32H7");
		LCD_DisplayText(13,106,"240*240");
...
void LCD_Test_Variable (void)
{
   ...
   LCD_SetTextFont(&CH_Font24);
	...
	LCD_DisplayText(0,10,"+int:");
	LCD_DisplayText(0,40,"-int:");

	LCD_SetColor(LIGHT_YELLOW);				// 设置画笔，亮黄色
	LCD_DisplayText(0, 80,"space:");
	LCD_DisplayText(0,110,"zero0:");

	LCD_SetColor(LIGHT_RED);					// 设置画笔	，亮红色
	LCD_DisplayText(0, 150,"+float:");
	LCD_DisplayText(0, 180,"-float:");
...
void  LCD_Test_Direction(void)
{
	for(int i=0;i<4;i++)
	{
      ...
      LCD_SetColor( 0xff9DD3A8);
      LCD_DisplayText(130,90,"LXB");
      LCD_DisplayText(130,130,"TECH");

      HAL_Delay(1000);	// 延时
	}
}
/*************************************************************************************************
*	函 数 名:	LCD_Test_ColorfulImage
*
*	函数功能:	显示彩色图片
*************************************************************************************************/
void LCD_Test_ColorfulImage(void)
{
	  LCD_DrawImageColorful( 0, 0, 240, 240, Image_yumemita);
      HAL_Delay(2000);
}
